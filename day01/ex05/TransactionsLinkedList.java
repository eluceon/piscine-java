...
public class TransactionsLinkedList implements TransactionsList {
	...
	@Override
    public Transaction remove(String stringUuid) {
        Node node = head.next;

        while (node != tail) {
            if (node.transaction.getUuid().toString().equals(stringUuid)) {
				node.prev.next = node.next;
				node.next.prev = node.prev;
                --size;
                return node.transaction;
	...
	@Override
	public boolean contains(String stringUuid) {
		Node node = head.next;
	...
	@Override
	public Transaction get(String stringUuid) {
		Node node = head.next;
